...
const VideoCall = forwardRef(({ roomId }, ref) => {
  ...
  const [cameraOn, setCameraOn] = useState(false)  // Start OFF
  const [micOn, setMicOn] = useState(false)        // Start OFF
  ...
  useEffect(() => {
    const startMedia = async () => {
      try {
        ...
        // Set initial track states to OFF (match React state)
        stream.getVideoTracks().forEach(track => {
          track.enabled = false
          console.log('📹 Initial video track set to OFF')
        })
        stream.getAudioTracks().forEach(track => {
          track.enabled = false
          console.log('🎤 Initial audio track set to OFF')
        })
  ...
  // Camera Toggle Function - Uses React state directly
  const toggleCamera = () => {
    ...
    if (videoTracks.length === 0) {
      console.error('❌ No video tracks found for camera toggle')
      return
    }

    // Toggle using React state (reliable)
    ...
    console.log('✅ Camera is now:', newState ? 'ON' : 'OFF')
  }

  // Microphone Toggle Function - Uses React state directly
  const toggleMic = () => {
    ...
    if (audioTracks.length === 0) {
      console.error('❌ No audio tracks found for mic toggle')
      return
    }
    ...
    setMicOn(newState)
    console.log('✅ Microphone is now:', newState ? 'ON' : 'OFF')
  }
